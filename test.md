...
If there is a load balancer in front of the cluster doing pass-through, the certificates need to know about the VIP front-end’s hostname.
Subject Alternative Name extensions should be set on many of the CSRs using the following configuration entry in the defaults.yaml file:
...
Make sure all the nodes in the cluster are all actively participating and are not decommissioned or otherwise disabled.  If any python errors occurred, the script crashed and did not complete.  Remove any offending nodes from the cluster and re-run the command
  ...
  1.####	Run the script to enable TLS:
Using this command:
